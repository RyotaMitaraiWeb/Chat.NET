...
describe('usePagination', () => {
  it('Returns correct array', () => {
    const pagination = renderHook(() => usePagination(2)).result.current;
    expect(pagination.pages.map((p) => p.page)).toEqual([1, 2]);
  });

  it('Correctly updates the selected page', () => {
    const pagination = renderHook(() => usePagination(2)).result;
  ...
  it('Does not update beyond limits', () => {
    const pagination = renderHook(() => usePagination(5, 3)).result;
    act(() => pagination.current.setPage(0));
    expect(pagination.current.pages[2].isSelected).toBe(true);
